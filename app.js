...
document.body.insertBefore(
  ratingSelect,
  document.getElementById("fetch-problems")
);

// Event listener for fetching problems based on selected rating
document
  .getElementById("fetch-problems")
  .addEventListener("click", async () => {
    ...
    const problems = data.result.problems
      .filter((problem) => problem.rating == selectedRating)
      .slice(0, 50);
    ...
    const submissionsByProblem = userSubmissionsData.result.reduce(
      (acc, submission) => {
    ...
    problems.forEach((problem) => {
      ...
      if (
        ...
        submissionsByProblem[problemId].includes("OK")
      ) {
        acceptedCount++;
      }
    });
    ...
    problems.forEach((problem, index) => {
      ...
      const verdicts = submissionsByProblem[problemId]
        ? submissionsByProblem[problemId].join(", ")
        : "No Submissions";
